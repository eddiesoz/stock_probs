...
- `M07`, `M08`, and `ASTRA-FINAL` are **Pending**. `EXP-M00`, `EXP-M01`, and `EXP-M02` are completed; `EXP-M03` through `EXP-M08` and final `EXP-FINAL` remain pending.
- A task may move to **Completed** only after every acceptance item, independent QA verification, repair history, reviewer, export, secret review, local commit, push, and Git remote revision verification is recorded. Implementation claims alone never change status. `EXP-M02` moved to **Completed** only through the later named repair receipt, not at the earlier missing-review/missing-timing point. A physical ARM64 performance result is never inferred from emulation.
...
7. Follow the canonical sequence exactly: `R-M00-1` -> `EXP-M00` (**Completed**) -> `M01` (**Completed**) -> `EXP-M01` (**Completed**) -> `M02` (**Completed**) -> `EXP-M02` (**Completed**) -> `M03` (**In progress**) -> `EXP-M03` (**next gate**) -> ... -> `M06`/`EXP-M06` -> `M07` QA/docs -> `EXP-M07` -> `M08` walkthrough QA/docs -> `ASTRA-FINAL` -> repairs/retests until the result is `Accepted` -> `EXP-M08` -> `EXP-FINAL`. The final gate is allowed only after Astra acceptance and all repair retests.
...
- **Post-milestone gate:** `EXP-M01` is **Completed** at SHA `5424fa3e22d9229d038d376512e59b3f35c97e78`; M02 and `EXP-M02` are subsequently completed through the late repair receipt, and `M03`/`EXP-M03` is the current next gate.
...
- **Post-milestone gate:** `EXP-M02` is **Completed** through `EXP-M02-REPAIR-1`; `M03` is **In progress** and `EXP-M03` is the next gate. This docs gate did not perform the export, commit, push, or remote verification.
...
- **Evidence state:** The historical independent retest passed `R-M03-1`, `R-M03-2`, and `R-M05-3`. The exact M03 receipts below cover provider, session, numerical, evaluation, browser/MCP, packaging, loopback, live, and explicitly emulated-ARM64 scopes. `R-M03-3`–`R-M03-22` are **Completed** after their recorded failures/skips and named repairs/retests; `R-M03-23` is **Pending** because actual screen-reader evidence is unavailable and deferred to M04/M06. M03 is **In progress** only for `EXP-M03`.
...
The exact rows below preserve the supplied task labels. `Completed` means the named behavior or gate is complete in scope; it does not complete M03 or its export. Initial failures remain in the ledger and are not erased by later regression passes.
...
- **Export/Git/reviewer:** `EXP-M03` is **Pending**. No M03 full-session export, secret review, local commit, push, exact Git remote revision verification, or named export reviewer is evidenced. The current docs reviewer may reconcile these records but does not substitute for the missing export gate. `R-M03-23` remains a later accessibility limitation rather than a blocker to the M03 provider/model scope.
- **Post-milestone gate:** `EXP-M03` must consume the completed M03 QA/docs record, overwrite and review the full `SESSION-EXPORT.md`, record secret review, commit, push, and verify the exact remote revision. Unavailable ARM64 hardware, screen-reader, or connectivity fields remain visible; no result is inferred from implementation claims.
