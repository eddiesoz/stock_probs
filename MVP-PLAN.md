...
- `M04` is **Completed** for its exercised functional scope after named independent repairs; only `EXP-M04` is **In progress**. `R-M04-30` remains **Pending** because actual screen-reader evidence is unavailable and deferred to M06; it blocks final accessibility/release acceptance, not the exercised M04 scope. `M05` and `M06` remain **Blocked** by their own recorded findings and missing release gates.
- `M07`, `M08`, `ASTRA-FINAL`, and `EXP-FINAL` are **Pending**. `EXP-M00`, `EXP-M01`, `EXP-M02`, and `EXP-M03` are completed; `EXP-M04` is the current in-progress export gate and later exports remain pending.
- A task may move to **Completed** for its declared scope only after every acceptance item in that scope, independent QA verification, repair history, and reviewer are recorded. A milestone's full acceptance/checkpoint additionally requires its export, secret review, local commit, push, and exact Git remote revision verification. A milestone explicitly marked **Completed for exercised scope** may retain a separately recorded deferred/unavailable later-release row and an `In progress` export gate; neither is counted as a pass, and both can block final release acceptance. Implementation claims alone never change status. `EXP-M02` moved to **Completed** only through the later named repair receipt, not at the earlier missing-review/missing-timing point. A physical ARM64 performance result is never inferred from emulation.
...
- **Export/Git/reviewer:** At the time of `R-M00-2`, no export/commit/push was performed by that documentation-only repair and `EXP-M02` had not yet been completed. The later coordinator records `EXP-M00`, `EXP-M01`, `EXP-M02`, and `EXP-M03` are **Completed** at the exact checkpoint receipts recorded below; those later results do not rewrite the historical timing. `EXP-M04` is **In progress** and later exports remain pending. Reviewer for `R-M00-2`: `LUNA MAX docs` self-review; independent implementation QA was not performed by that historical docs repair.
...
The source task IDs above are evidence locators, not acceptance reviewers. `R-M01-3` and `R-M01-4` now have current independent scoped pass records; the other listed aliases remain pending, and no later repair may reuse one of these fresh IDs for a different defect. `R-M01-5` through `R-M01-15` are current coordinator-assigned M01 records in the M01 register, not additional collision aliases. The `R-M04-2`/`R-M04-3` values here are source-qualified historical aliases from `R-M00-1`; they are retained without being merged into the later source-qualified M04 QA labels carrying the same text. The current M04 screen-reader obligation is `R-M04-30`.
...
5. Run the independent `LUNA MAX QA` and `LUNA MAX docs` waves only after that wait and review. Use one to three agents according to genuinely independent scopes, launch independent QA/docs work in parallel, and keep dependent work sequential. QA is verification-only: it denies repair and delegation conceptually; it does not repair implementation/configuration or hand acceptance to a builder. Docs finalizes only after consuming the completed QA record and must not fill a missing QA result with an implementation claim.
...
The coordinator assigns the exact task ID. Each implementation or repair build wave runs exactly three concurrent build lanes and never more than three builders, waits for handoff, then runs the independent QA/docs gates; a QA/docs wave may use one to three agents according to genuinely independent scopes, and docs finalizes only after consuming the completed QA record. The listed acceptance evidence is required before status can become **Completed**, and the matching export gate is required before the milestone is released onward. M08 is the explicit ordering exception: its artifact/QA/docs result is reviewed by Astra first, then `EXP-M08` checkpoints that final walkthrough.
...
- **Status:** Completed (exercised functional scope only); only `EXP-M04` is **In progress**.
...
- **Export/Git/reviewer:** `EXP-M04` is **In progress**. No M04 full-session export, secret review, local commit, push, or exact remote-main verification is supplied or performed by this docs gate. The final functional artifact's revision is not an M04 checkpoint.
- **Post-milestone gate:** `M04` exercised scope is **Completed**; `EXP-M04` must still overwrite and sanitize the full `SESSION-EXPORT.md`, complete secret review, commit, push, and verify exact remote `main`. `R-M04-30` blocks final accessibility/release acceptance and is deferred to M06.

#### `EXP-M04` current in-progress export gate

- **Status:** In progress.
- **Owner/phase:** coordinator export gate; this `LUNA MAX docs` gate records the supplied QA receipt but does not execute the export gate. No independent export reviewer was supplied.
...
- **Change summary:** the full-session export, secret review, local commit, push, and exact remote-main verification remain future checkpoint work; no future result is inferred from the M04 functional artifact or dirty revision.
- **Evidence ledger:**

  | Evidence ID | Requirement/check | Environment, UTC time, commit | Result, artifact, reviewer, limitation |
  | --- | --- | --- | --- |
  | `EXP-M04-E1` | Full-session `SESSION-EXPORT.md` export, parse, and inventory | Current native x86_64 documentation worktree; command/time/commit unavailable because not run | **Unavailable**; no export artifact supplied or created by this docs gate; export reviewer not supplied. |
  | `EXP-M04-E2` | Full secret review of the M04 export | Same; no M04 export available | **Unavailable**; no secret-review result or credential claim is made; reviewer not supplied. |
  | `EXP-M04-E3` | Local commit and push of the reviewed export | Same; no Git operation performed by this docs gate | **Unavailable**; no commit or push is claimed. |
  | `EXP-M04-E4` | Exact remote-main revision verification and named checkpoint | Same; no M04 checkpoint exists | **Unavailable**; no artifact/link or remote result is claimed. |

- **Repairs/history:** `R-M04-30` remains open for M06; the initial functional failures, repairs, retests, and final `R-M04-55` receipt remain in the M04 ledger above.
- **Limitations:** no native/physical ARM64 performance, actual screen-reader result, CI result, export, secret review, commit, push, or remote verification is claimed for `EXP-M04`.
- **Reviewer/export/revision:** named independent reviewer for this future export gate is unavailable/not supplied; `LUNA MAX docs` self-review is limited to this documentation record and is not export acceptance. No checkpoint SHA exists for `EXP-M04`.
...
- **Acceptance evidence:** [ ] the official MCP configuration is discoverable, runs headless locally, and completes an application interaction; [ ] a clean package installation and `make check` pass on native x86-64 and on local native or labelled emulated ARM64; [ ] loopback startup, Node/Chromium, desktop/mobile browser tests, and functional/tool evidence run locally on both target architectures; [ ] browser regressions are checked in and run against a deterministic local app; [ ] a deliberate local failure proves the Make/scripts gate exits nonzero and stops; [ ] `.github/workflows/ci.yml` is absent; [ ] CSV and JSON exports are tested for safe content; [ ] cross-architecture backups and trust-key missing/wrong-key fail-closed cases pass; [ ] test artifacts identify task and commit context; [ ] no test depends on tracked `burry_env/` or local `.venv/`; [ ] the mandatory native-x86 performance and UI rows below are independently passed or explicitly recorded as `Fail`, `Skipped`, or `Unavailable`; [ ] physical ARM64 low-resource performance is marked `Unavailable` unless local native ARM64 hardware exists; [ ] the pinned Ingenium Ponytail closure is configured and read-only `/ponytail-review` is independently reviewed, with only `SOL HIGH` repairs; [ ] the project documentation skill is created and validated after a required pre-change commit using Ingenium's structure, index, metadata, and learnings conventions; [ ] least-privilege Ingenium MCP credential-file binding, restart, project registration, repository-sync dry-run/apply/no-drift, and credential-free onboarding are independently recorded.
- **Verification:** [ ] clean-environment package build/install and `make check` on current native x86-64; [ ] local native ARM64 run if hardware becomes available, otherwise local QEMU/OCI multi-arch run explicitly labelled emulated; [ ] intentional-failure local-gate test; [ ] headless official MCP application interaction; [ ] loopback check; [ ] Node/Chromium browser regression on 360/390/768/1280/1440 desktop/mobile viewports; [ ] separate actual assistive-technology run; [ ] the per-check native-x86 performance/UI artifacts below; [ ] native ARM64 performance evidence only when hardware exists, otherwise an explicit `Unavailable`; [ ] cross-architecture backup/key lifecycle run; [ ] pinned Ponytail provenance/configuration, parent restart, read-only review, SOL-only repair, and independent QA; [ ] pre-change commit, documentation skill structure/index/metadata/learnings validation, and independent review; [ ] authorized workspace configuration, parent restart, least-privilege credential-file binding, MCP registration, repository-sync dry-run/apply/no-drift, and credential-free onboarding evidence. Missing configuration, restart, workspace, credentials, samples, baseline, bound, artifact, or reviewer remains `Pending`, `Blocked`, or `Unavailable`; it is not inferred.
...
These are future acceptance rows inside `M06`, not current results. `M06` remains
**Blocked**. The run must use native x86_64 Linux, a fresh isolated deterministic fixture
...
Each row below has task ID `M06`. Its current evidence result is **Unavailable** because
this documentation reconciliation does not run performance QA. A later failure receives
an unused exact `R-M06-<n>` repair ID only after the coordinator assigns it; no row may be
renamed or closed from an implementation claim.
...
| --- | --- | --- | --- | --- |
| `M06` | Proposed protocol | Fresh isolated fixture runtime | Start a new app process with the deterministic fixture, isolated port and temp directory; record fixture/cache identity, process tree, CPU architecture, Python/runtime, commit, and cleanup. | **Unavailable**; no run claimed. |
| `M06` | Proposed protocol | Five warmups plus `>=30` measured requests | Exclude exactly at least five warmups from statistics, then record at least 30 raw measured requests for each designated workload and declared concurrency level. Fewer samples are `Fail` or `Unavailable`, never a pass. | **Unavailable**; no samples claimed. |
| `M06` | Existing numeric threshold | Process RSS `<500 MiB` | Measure peak process RSS for the isolated app/runtime and identify whether child processes are included; retain raw samples, units, measurement method, and peak. | **Unavailable**; no RSS result claimed. |
| `M06` | Existing numeric threshold | Defined fixture/cache-hit forecast p95 `<1 s` | Declare the forecast route, input, fixture, and cache-hit condition; report every measured elapsed time plus p50/p95/max, errors, and cache state. | **Unavailable**; no forecast result claimed. |
| `M06` | Existing numeric threshold | Indexed 100,000-history query p95 `<250 ms` with `>=10` repetitions and an index plan | Seed the exact 100,000-row fixture, record the index/schema and query plan, run at least 10 repetitions in the isolated runtime, and report raw timings plus p50/p95/max. Missing the index plan or repetitions is `Fail` or `Unavailable`. | **Unavailable**; no history result claimed. |
| `M06` | Proposed numeric threshold | 60-second idle CPU `<=1 core-percent` | After readiness and with no requests, measure the app for 60 seconds; record raw CPU time/samples, sampling interval, process scope, normalization, and max. The proposed bound applies to mean normalized CPU `<=1 core-percent`; it does not replace the existing qualitative `negligible` requirement. | **Unavailable**; no CPU result claimed. |
| `M06` | Proposed protocol/bound | Concurrency elapsed | Declare concurrency levels (at least 1, 4, and 8 where the runtime supports them), perform the warmup/sample protocol at each level, and record batch wall-clock elapsed, per-request p50/p95/max, errors, and peak RSS. A predeclared bound must be present; a single-request result cannot substitute for concurrency evidence. | **Unavailable**; no concurrency result or bound claimed. |
| `M06` | Proposed numeric threshold | Readiness within `20 s` | From a fresh isolated start, record process start, readiness command, readiness timestamp, retries (none hidden), and total elapsed time. The readiness bound is `20 s`; an unavailable or timed-out readiness check blocks the row. | **Unavailable**; no readiness result claimed. |
| `M06` | Proposed baseline/bound | Package size and clean build time | Record the exact package artifact bytes and clean isolated build wall time, fixture/tool versions, and a named baseline commit. A missing baseline or bound is `Unavailable`, not a pass. | **Unavailable**; no package/build baseline claimed. |
| `M06` | Proposed baseline/bound | Backup and restore duration and bounds | Use a named representative fixture and artifact byte size; measure backup and restore wall time, peak RSS, bytes, and verification outcome, and record the predeclared duration bounds. Missing fixture size, bound, or restore verification is `Unavailable` or `Fail`. | **Unavailable**; no backup/restore result claimed. |
| `M06` | Proposed numeric thresholds | Static shell/assets `<96 KiB` and designated response `<8 KiB` | Declare whether bytes are raw or transfer bytes, measure the production static shell/assets and the designated response separately, record each file/response and compression state, and apply the strict `<` bounds. | **Unavailable**; no byte result claimed. |
| `M06` | Proposed browser budgets | Render, interaction, layout, and request budgets | Before the run, pin a browser manifest with render p95 (proposed FCP/ready-to-use `<=2 s`), interaction p95 (proposed control-to-visible-state `<=250 ms` for local non-network actions), layout (proposed CLS `<=0.1` plus zero overlap/overflow at 360/390/768/1280/1440), and request (zero unexpected requests, no direct provider/SQLite access, no request outside the declared baseline, and the designated response `<8 KiB`). Record raw traces, timings, request names/count/bytes, viewport, and console result. These proposed budgets are not current browser results. | **Unavailable**; no browser budget result claimed. |
| `M06` | Evidence rule | One structured artifact per check | Each artifact must contain task ID `M06`, row name, clean fixture identity, process/port/temp isolation, native x86_64 environment, commit, UTC start/end, command, warmup/measured samples, p50/p95/max where relevant, raw RSS/CPU/request/bytes/timing data, existing-versus-proposed classification, result (`Pass`, `Fail`, `Skipped`, or `Unavailable`), limitation, artifact path, and named independent reviewer. | **Unavailable**; no artifacts claimed. |
| `M06` | Architecture limitation | ARM64 performance field | Native/physical ARM64 performance may be measured only on available local native ARM64 hardware. QEMU/OCI/emulated ARM64 may verify packaging, runtime, functional, build, or tool behavior, but any performance field remains `Unavailable`; no ARM64 resource result is inferred. | **Unavailable** unless a qualifying native run exists. |
...
IDs. `M06` remains **Blocked**, and the rows below are not current completion claims.

| Task ID | Row status | Owner/phase | Preconditions and required check | Current evidence result and limitation |
| --- | --- | --- | --- | --- |
| `M06` | **Blocked** | `LUNA MAX docs` records the requirement; `SOL HIGH` owns any repair; `LUNA MAX QA` independently retests | Vendor/configure the pinned Ingenium Ponytail closure, complete the required parent restart, and run read-only `/ponytail-review` after every implementation boundary. Use the exact clean/finding format above and restrict the review to overengineering. | **Unavailable** for M06 acceptance: no configuration, restart, review, SOL repair, or independent QA receipt is supplied. Ponytail is not correctness, security, accessibility, or performance evidence. |
| `M06` | **Pending** | `LUNA MAX docs` plus independent validation | Make the required pre-change commit, then create and validate the project documentation skill using Ingenium's skill structure, index, metadata, and learnings conventions | **Unavailable** for M06 acceptance: no pre-change commit, skill, validation, index, or reviewer receipt is supplied. |
| `M06` | **Blocked** | `LUNA MAX docs` records credential-free onboarding; independent QA verifies it | Configure least-privilege Ingenium MCP credential-file binding, restart, register the project, run repository-sync dry-run/apply/no-drift, and record onboarding without credential values | **Unavailable** for M06 acceptance: authorized workspace configuration, restart, and independent onboarding evidence are not supplied. No endpoint or token value is recorded. |
...
rows are actually run. A missing precondition remains visible and cannot be replaced by
...
| --- | --- | --- |
| Deny-by-default least privilege | Adopted in ownership, credential, and permission policy. | M06 credential-file binding, restart, and onboarding remain **Blocked** until independently evidenced. |
| Source-versus-live evidence separation | Adopted in every ledger: historical/source reports never become current live acceptance. | Each M06 row must name its live environment, commit, UTC window, and reviewer; no current performance run is claimed. |
...
| Bounded process, port, and temporary artifacts | Adopted as the isolation and cleanup requirement for performance and local gates. | M06 must record process/port/temp bounds and cleanup per structured artifact; current evidence is **Unavailable**. |
...
direct-route SQL, and replica rate limiting. Authored-documentation taxonomy implementation
remains **Pending** after the future clean pre-skill `EXP-M04` checkpoint: the required
pre-change commit and independent skill validation come first, and this reconciliation
creates no skill and claims no clean checkpoint.
...
7. The coordinator marks a milestone completed for its declared scope only when every acceptance field in that scope, independent QA/docs review, and reviewer field pass. Full milestone/checkpoint acceptance additionally requires the matching `EXP-M00`–`EXP-M08` export, secret review, push, and exact Git remote revision verification. An explicitly deferred or unavailable later-release field, or an `In progress` export gate for a milestone marked **Completed for exercised scope**, remains visible, is not counted as a pass, and blocks final acceptance. The supplied x64 SHA context is not dual-architecture or `EXP-M00` proof.
...
- `EXP-M00` through `EXP-M08` and `EXP-FINAL` each have a reviewed full-session `SESSION-EXPORT.md`, recorded export revision, secret review, commit, push, exact Git remote revision verification, and no hidden unavailable or failed field; the final learning synthesis is recorded before `EXP-FINAL`.
- The final learning synthesis deeply analyzes sanitized chat/run evidence, creates or changes only justified reusable Stock Probability skills through validated/indexed `skill-maintenance` work, and records observations before `EXP-FINAL`; authored-documentation taxonomy remains Pending until its clean pre-skill `EXP-M04` checkpoint, pre-change commit, and independent validation exist.
