...
1. Capture the exact task ID, revision, dirty state, native architecture, command, reviewer, and UTC
   window before interpreting a result.
2. Run only the requested profile and preserve its first failure. Follow
   [`references/gate-protocol.md`](references/gate-protocol.md) for isolation and reruns.
...
- Fail closed on a missing row, artifact, reviewer, command, revision, timestamp, or cleanup proof.
